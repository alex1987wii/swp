...
DSP_INC= -I$(UNILIB_PATH)/libbitdsp -L$(UNILIB_PATH)/libbitdsp -I/$(UNILIB_PATH)/dspadapter/include -L/$(UNILIB_PATH)/dspadapter -L/$(UNILIB_PATH)/timers -L$(UNILIB_PATH)/annal -L$(UNILIB_PATH)/circbuf -L$(UNILIB_PATH)/crc16
...
lib/ldsp.so: ldsp/ldsp.c
	$(CC) -Wall -shared -o $@ -fPIC $^ $(CFLAGS) -ldspadapter -lbitdsp -lpthread -ltimers -lannal -lcircbuf -lcrc16 $(DSP_INC) -DCONFIG_PROJECT_U4
	
lib/lnondsp.so: lnondsp/lnondsp.c
	$(CC) -Wall -shared -o $@ -fPIC $^ $(CFLAGS) -lbitnondsp -lpthread -ltimers -lannal -lcircbuf -lbluetooth -ldspadapter -lbitdsp -lcrc16 $(NONDSP_INC) -DCONFIG_PROJECT_U4
